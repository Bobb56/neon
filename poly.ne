#détournement de la sortie pour le test
...
function print(...) do
    for (i, 0, len(__local_args__)) do
        OUT += str(__local_args__[i])
        if (i < len(__local_args__)-1) then OUT += ' ' end
    end
    OUT += '\n'
end
...
function display_factorised(a, roots) do
    ...
    if (len(roots) == 0) then
        print_sov("The polynom can't be factorised")
        return ()
    end
...
function main() do
    POLYNOM = "(x+4)*(x-12)" #input("Entrez l'expression d'un polynome de degre deux : ")
    analysis(f)
    print = print_sov
    output = output_sov
    #print(OUT)
    #assert (OUT == "Your developped form is :\nx²-8x-48\nHere are the two roots : -4 12\n(x+4)(x-12)\n")
end
...
$
1+2+3+45 - 112 + 18 + x*x*x - x*x*x + 3*x*(x-1+1) - 2 * x * x
$
