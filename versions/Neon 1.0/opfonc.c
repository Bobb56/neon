...
char* _affect(char* op1, unsigned nat1, char* op2, unsigned nat2,int* typeRetour)
{
  ...
  if (inList || nat1==TYPE_LISTINDEX[0])// si la variable existe deja on la supprime avant
  {
    ...
    else if (nat1==TYPE_VARIABLE[0])
    {
      ...
      {
        if ((TYPES.tab[k] != TYPE_LIST[0] && TYPES.tab[k] != TYPE_TEMPLIST[0] && typeVar != TYPE_LIST[0] && typeVar != TYPE_TEMPLIST[0]) || (TYPES.tab[k]==TYPE_LIST[0] && typeVar==TYPE_TEMPLIST[0]) || (TYPES.tab[k]==TYPE_TEMPLIST[0] && typeVar==TYPE_LIST[0]) || (TYPES.tab[k]==TYPE_LIST[0] && typeVar==TYPE_LIST[0]))//si la variable est le type de variable supprimée
        {
