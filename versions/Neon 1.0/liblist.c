/*
Cette bibliothèque ne fait que partiellement partie du logiciel Neon 1.0
Elle provient du logiciel Neon 2.12 et a été adaptée pour fonctionner avec Neon 1.0
*/
...
void strlist_copy(strlist* list, const char** tab, unsigned len)
{
...
void intlist_copy(intlist* list, const int* tab, unsigned len)
{
    // copie du tableau dans le tas
...
void listlist_copy(listlist* list, const strlist* tab, unsigned len)
{
    // copie du tableau dans le tas
    strlist* res = err_malloc(sizeof(strlist)*len);
    strlist temp;
    for (unsigned i = 0 ; i < len ; i++)
    {
        // ---- copie -----
        char** res2 = err_malloc(sizeof(char*)*tab[i].len);
        for (unsigned j = 0 ; j < tab[i].len ; j++)
        {
            res2[j] = tab[i].tab[j];
        }
