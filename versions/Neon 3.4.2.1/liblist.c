...
Stack* stack_create(int len)// crée une pile de taille au moins len
{
    ...
    stack->stack_ptr = stack->start = err_malloc((1 << stack->capacity) * sizeof(void*));

    return stack; //renvoie la structure
}


void stack_check_for(Stack* stack, int size) {
    if (stack->start + (1 << stack->capacity) < stack->stack_ptr + size) {
        int len = stack->stack_ptr - stack->start;
        stack->start = err_realloc(stack->start, (1 << ++stack->capacity) * sizeof(void*));
        stack->stack_ptr = stack->start + len;
    }
}
