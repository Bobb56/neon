...
NeObj eval_aux(Tree* tree) {
    ...
    else if (tree->type == TYPE_FONCTION)
    {
        ...
        else if (NEO_TYPE(tree->data) == TYPE_USERFUNC || NEO_TYPE(tree->data) == TYPE_USERMETHOD)
        {
            ...
            NeObj tree_data_sov = tree->data;
            tree->data = NEO_VOID; // pour que les appels futurs n'oublient pas d'évaluer la fonction

            NeList* liste = treeToList(arguments); // expérience : pour les méthodes, la seule différence avec les fonctions est qu'à la fin, on met le premier argument dans le premier argument (fin tu m'as compris)
            NeObj* self = NULL;

            // si c'est une méthode, on récupère l'adresse de l'objet à modifier
            if (NEO_TYPE(tree_data_sov) == TYPE_USERMETHOD)
                self = get_address(arguments->sons[0]);
            ...
            NeObj ret;
            if (NEO_TYPE(tree_data_sov) == TYPE_USERMETHOD)
                ret = callUserMethod(fun, self, liste, neo_local_args);
            else
                ret = callUserFunc(fun, liste, neo_local_args);

            tree->data = tree_data_sov;
            neobject_destroy(tree->data);
            tree->data = NEO_VOID;

            nelist_destroy(liste);
...
int exec_aux(Tree* tree) {
    ...
    for (int inst=0 ; inst < tree->nbSons ; inst++)
    {
        ...
        else if (tree->sons[inst]->type == TYPE_FUNCTIONDEF)
        {
            ...
            replace_var(var, neo_copy(tree->sons[inst]->data));
        }
