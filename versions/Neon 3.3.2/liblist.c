...
// fonctions pour la stack de execval



Stack* stack_create(int len)// crée une liste d'entiers
{
  Stack* stack = err_malloc(sizeof(Stack));//crée la structure
  
  stack->capacity = 0;
  
  while (1 << stack->capacity < len)
    stack->capacity++;
  
  stack->tab=err_malloc((1 << stack->capacity) * sizeof(void*));//initialise le tableau de longueur len avec de zéros
    
  stack->len=0;//initialise la bonne longueur
  return stack;//retourne la structure
}




void stack_check_for(Stack* stack, int n) { // on suppose que la pile a une taille initiale suffisamment grande pour qu'un seul agrandissement suffise
  if (1 << stack->capacity < stack->len + n)
    stack->tab = err_realloc(stack->tab, (1 << ++stack->capacity) * sizeof(void*));
}


void stack_extend(Stack* stack, int size) {
  stack_check_for(stack, size);
  stack->len += size;
}
