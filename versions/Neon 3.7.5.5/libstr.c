...
_Bool has_strict_prefix(char* string, char* prefix) {
    ...
    _Bool bo = (strcmp(string, prefix) == 0);
...
strlist* get_all_modules(void) {
    ...
    for (int i=0 ; i < NOMS->len ; i++) {
        ...
        if (prefix != NULL && ADRESSES->tab[i]->type != TYPE_EMPTY && !strlist_inList(modules, prefix)) {
...
int function_module(char* module, char* function) {
    ...
    if (n < 0 || (ADRESSES->tab[n]->type != TYPE_USERFUNC && ADRESSES->tab[n]->type != TYPE_USERMETHOD))
        return -1;
