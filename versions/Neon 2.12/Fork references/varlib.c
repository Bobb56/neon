...
NeObject* neo_copy(NeObject* neo)
{
    ...
    neo2->data = neo->data;
    neo2->refc = neo->refc; // le compteur de références est le même car c'est le même neo->data. Ainsi si un autre est libérén, ce compteur sera lui aussi modifié
...
void neobject_destroy(NeObject* neo, _Bool bo) // décrémente le pointeur, supprime éventuellement tout l'objet et supprime éventuellement l'enveloppe
{
    ...
    {
        int autoref = neobject_autoref(neo);
    ...
    if (bo) // par contre peu importe la valeur de refc, l'enveloppe est systématiquement supprimée (sauf si elle est réutilisée)
...
int neobject_autoref_aux(NeObject* neo) // fonction auxiliaire qui ne modifie pas neo->refc
{
    ...
    else // on n'est jamais passé par là
    {
        ...
        *(neo->refc) = (unsigned)-3; // pour savoir quand on revient à un pointeur déjà parcouru qui n'est pas le pointeur recherché
        ...
        if (neo->type == TYPE_LIST) // on additionne les autoref de chacun des indices de liste
        {
            ...
            for (int i = 0 ; i < list->len ; i++)
            {
                somme += neobject_autoref_aux(list->tab[i]);
            }
        }
        else if (neo->type == TYPE_CONTAINER) // on additionne les autoref de chacun des champs
        {
            ...
            for (int i = 0 ; i < c->data->len ; i++)
            {
                somme += neobject_autoref_aux(c->data->tab[i]);
            }
        }
...
int neobject_autoref(NeObject* neo) // fonction qui calcule combien de fois neo fait-il référence à lui-même
{
    unsigned sov = *(neo->refc);
