...
extern NeObject* eval(Tree* tree);
...
void partialRestore(ptrlist* varsToSave, ptrlist* sov_vars_to_save) {
    NeSave* save;
    while (process_cycle->process->varsToSave->tete != sov_vars_to_save) {
        save = ptrlist_pop(process_cycle->process->varsToSave);
    ...
    switch (tree->type) {
        ...
        case TYPE_LIST:
        {
            ...
            for (int index = 0; index < l->len ; index++) {
                ...
                l->tab[index] = eval_prolog(tree->sons[index], 1);

                if (CODE_ERROR != 0) {
                    for (int i=0 ; i < index ; i++)
                        neobject_destroy(l->tab[index], true);
                    break;
                }
            }
        ...
        case TYPE_FONCTION:
        {
            ...
            if (tree->data->type == TYPE_FONCTION)
            {
                ...
                NeList* l = nelist_create(tree->sons[1]->nbSons);
                ...
                {
                    tree->data = NULL;
                    l->tab[ext_index] = eval_prolog(tree->sons[1]->sons[ext_index], 0);
                ...
                for (;ext_index < tree->sons[1]->nbSons ; ext_index++)
                {
                    ...
                    l->tab[ext_index] = eval_prolog(tree->sons[1]->sons[ext_index], 1);
                    tree->data = tree_data_sov;
            ...
            else if (tree->data->type == TYPE_USERFUNC || tree->data->type == TYPE_USERMETHOD)
            {
                ...
                NeList* liste = nelist_create(arguments->nbSons);

                // variables à sauvegarder : fun, liste, ext_index et arguments
                NeObject* tree_data_sov = tree->data;

                for (int ext_index = 0 ; ext_index < liste->len ; ext_index++) {
                    
                    tree->data = NULL; // pour que les appels futurs n'oublient pas d'évaluer la fonction
                    if (ext_index == 0 && tree_data_sov->type == TYPE_USERMETHOD) {
                        liste->tab[ext_index] = eval_prolog(arguments->sons[ext_index], 0);
                    }
                    else {
                        liste->tab[ext_index] = eval_prolog(arguments->sons[ext_index], 1);
                    }
                    tree->data = tree_data_sov;
                ...
                NeObject* args = gc_extern_neo_list_convert(liste);
...
void exec(Tree* tree) {
    // on crée une userfunc qui exécute le code demandé, puis on la lance avec eval
    UserFunc myFunction = (UserFunc) {
        .args = NULL,
        .nbArgs = 0,
    ...
    NeObject* myFunctionObj = err_malloc(sizeof(NeObject));
    
    *myFunctionObj = (NeObject) {
        .data = (NeObjDat) {
            .ptr = &myFunction,
            .refc = NULL
        },
    ...
    Tree tree_func = (Tree) {
        ...
        .label1 = NULL,
        .label2 = 0,
        .sons = (Tree*[2]) {NULL, &son1},
        .nbSons = 2,
        .capacity = 0,
        .data = myFunctionObj
    };

    NeObject* res = eval(&tree_func);
    ...
    for (int inst=0 ; inst < tree->nbSons ; inst++)
    {
        switch (tree->sons[inst]->type) {
            ...
            case TYPE_STATEMENTFOR:
            {
                ...
                NeObject* valeur = eval_prolog(tree->sons[inst]->sons[0]->sons[1], 1);
                ...
                NeObject* tempMax = eval_prolog(tree->sons[inst]->sons[0]->sons[2], 1);
