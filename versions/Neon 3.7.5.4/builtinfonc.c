...
NeObject* _help_(NeList* args)
{
    ...
    else if (args->tab[0]->type == TYPE_STRING && strcmp(neo_to_string(args->tab[0]), "modules") == 0) {
    ...
    // aide à propos d'objets en général
        ...
        for (int i = 0 ; i < args->len ; i++) { // boucle pour afficher l'aide de chaque objet
            ...
            else if (args->tab[i]->type == TYPE_STRING && is_module(neo_to_string(args->tab[0]))) {
                printString("Module ");printString(neo_to_string(args->tab[0]));printString(" :");newLine();
                for (int i=0 ; i < NOMS->len ; i++) {
                    if (has_strict_prefix(NOMS->tab[i], neo_to_string(args->tab[0])) && ADRESSES->tab[i]->type != TYPE_EMPTY) {
...
NeObject* _failwith_(NeList* args)
{
    setColor(RED);
...
NeObject* _list_comp_(NeList* args)
{
    ...
    {
        NeObject* bo = eval_prolog(cond, 1);


        if (CODE_ERROR != 0) {
            tree_destroy(cond);
            tree_destroy(val);
            neobject_destroy(liste, true);
            if (sov_FILENAME != NULL)
                err_free(sov_FILENAME);
            return NULL;
        }
        ...
        if (neoIsTrue(bo)) {
            neo_list_append(liste, eval_prolog(val, 1));
...
NeObject* _raise_(NeList* args)
{
    EXCEPTION = neo_to_string(args->tab[1]);
    CODE_ERROR = -get_exception_code(args->tab[0]);
    return NULL;
}
...
NeObject* _load_namespace_(NeList* args) {
    char* namespace = neo_to_string(args->tab[0]);
...
NeObject* _gc_(NeList* list) {
    gc_mark_and_sweep();
    return neo_none_create();
}


NeObject* _setColor_(NeList* args) {
    char* color = neo_to_string(args->tab[0]);
