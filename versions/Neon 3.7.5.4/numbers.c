...
extern int CODE_ERROR;
    ...
    {
        return (double) ( (long int)n1 % (long int)n2 );
    }
    
    Number number_eucl(Number nb1, Number nb2)
    {
        return (double)(nb1-(long int)nb1%(long int)nb2) / nb2;
    }
    ...
    {
        unsigned prec = (unsigned)b;
        int dec = pow(10,prec);

        return (double)round(a * dec)/((double)dec);
    }
    ...
    Number number_incr(Number n, int incr) {
        real_t real_incr = os_FloatToReal((float)incr);
        return real_allocate(os_RealAdd(n, &real_incr));
    }
