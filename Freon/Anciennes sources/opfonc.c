...
NeObject* _infEqual(NeObject* op1, NeObject* op2)
{
    ...
    {
        return neo_bool_create(number_infEq(neo_to_nb(op1), neo_to_nb(op2)));
    }
    else if (op1->type == TYPE_STRING && op2->type == TYPE_STRING)
    {
        return neo_bool_create(strInfEq(neo_to_string(op1), neo_to_string(op2)));
    }
    CODE_ERROR = 52;
    return NULL;
}
...
NeObject* _supEqual(NeObject* op1, NeObject* op2)
{
    if (op1->type == TYPE_NUMBER && op2->type == TYPE_NUMBER)
    {
        return neo_bool_create(number_supEq(neo_to_nb(op1), neo_to_nb(op2)));
    }
    else if (op1->type == TYPE_STRING && op2->type == TYPE_STRING)
    {
        return neo_bool_create(!strInf(neo_to_string(op1), neo_to_string(op2)));
    }
    CODE_ERROR = 53;
    return NULL;
}
...
NeObject* _inf(NeObject* op1, NeObject* op2)
{
    if (op1->type == TYPE_NUMBER && op2->type == TYPE_NUMBER)
    {
        NeObject* equal = _supEqual(op1,op2);
        if (CODE_ERROR != 0)
            return NULL;
        NeObject* res = neo_bool_create(!neo_to_bool(equal));
        neobject_destroy(equal,true);
        return res;
    }
    else if (op1->type == TYPE_STRING && op2->type == TYPE_STRING)
    {
...
NeObject* _sup(NeObject* op1, NeObject* op2)
{
    if (op1->type == TYPE_NUMBER && op2->type == TYPE_NUMBER)
    {
        NeObject* equal = _infEqual(op1,op2);
        if (CODE_ERROR != 0)
            return NULL;
        NeObject* res = neo_bool_create(!neo_to_bool(equal));
        neobject_destroy(equal,true);
