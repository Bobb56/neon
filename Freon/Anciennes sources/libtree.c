...
char* fonctionArgs(char* fonction, char* set)
{
    ...
    int i = len;
    _Bool inString1 = false, inString2 = false, isPotentiallyWord = false;

    while ((fonction[i] != set[0] || pile != 0) && i >= 0)
    {

        i--;
        

        if (isalnum(fonction[i]) || fonction[i] == '_' || fonction[i] == '\'' && !inString1 && !inString2)
            isPotentiallyWord = true;
        
        if (isPotentiallyWord && fonction[i] != '_' && fonction[i] != '\'' && !isalnum(fonction[i]))
            isPotentiallyWord = false;


        if (fonction[i] == '"' && !inString2 && !isPotentiallyWord)
            inString1 = !inString1;

        if (fonction[i] == '\'' && !inString1 &&!isPotentiallyWord)
            inString2 = !inString2;
    ...
    if (fonction[len-2] == set[0]) // fonction sans arguments
        return NULL;
    else
        return sub(fonction, i+1, len-1);
}
...
void createExpressionTreeAux(Tree* tree, strlist* tokens, intlist* types, intlist* ordre)
{
    ...
    if (tokens->len == 1) // traitement des cas de base de la fonction récursive
    {
        ...
        if (types->tab[0] == TYPE_FONCTION)
        {
            ...
            tree->data = evalAux(tree_aux, false); // on sous entend que la fonction portant ce nom existe déjà
