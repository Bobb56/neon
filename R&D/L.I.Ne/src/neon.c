...
void terminal(void) {
    while (true) {
        ...
        if (treelistLength(&tb, treeSntxTree(&tb, tb.entry_point)->treelist) == 1 && NeTree_isexpr(&tb, treelistGet(&tb, treeSntxTree(&tb, tb.entry_point)->treelist)[0]))
        {
            ...
            InterpContext* interp_context = InterpContext_create();
            NeObj res = launch_evaluation(&tb, exprtree, interp_context);

            InterpContext_aff(interp_context);

            InterpContext_destroy(interp_context);
        ...
        {
            InterpContext* interp_context = InterpContext_create();
            tb_launch_execution(&tb, interp_context);
            InterpContext_destroy(interp_context);
...
void liveExecFile(char* filename) {
    char* program = openFile(filename); // fonction dépendant du système cible
