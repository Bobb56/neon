...
/*
TODO:
- Handle in_word for precisely knowing when to start a '' string and when to test if wa_state is a final state
- Handle different class of words in wordautomaton
...
// For non word operators
// use: sousop_arr[char - 33]
...
void print_highlighted(char* text) {
...
int main(void) {
    char* text = "function f(x) do\nprint('Hello, World')\nend\n";
    print_highlighted(text);

    text = "# Ceci est un programme de test\nfor (i, 0, 5) do\n    bo = cond and bo\n    a' = 1.5*12 + 0x412\nend\n";
    print_highlighted(text);

    text = "$ commentaire multiligne\ncomplet$";
    print_highlighted(text);
