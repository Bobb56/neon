...
void parse(strlist* tokenAdd, intlist typeTok, Ast** ast, int offset) {
    ...
    for (int k=0 ; k < tokenAdd->len ; k++) // boucle pour chaque nouveau token ajoute. chaque tour de cette boucle est comme un tour de boucle normal sur tokens. cela permet d’eviter une boucle supplementaire                
    {
        ...
        finsTypesComposes(&debTok2, &debTok3, &lastDebTok, k, &nouvTokComp, &isPotentiallyPar, &isPotentiallyFonc, &foncStep, tokenAdd, typeTok, &nbPar, &isPotentiallyInst, &instStep, &listIndexStep, &isPotentiallyList, &isPotentiallyListIndex, &nbCro, &isPotentiallyBlock, &nbAcc, &foncParDeb, &indexParDeb, &listeParDeb, ast, offset);
    ...
    if (tokenAdd->len > 0) {
        ...
        while (plusieursTokens) { // si on a plus d'un token composé ou simple
            ...
            if (ast[i_act]->type==TYPE_OPERATOR && tokenAdd->tab[i_act][0] == '-' && tokenAdd->tab[i_act][1] == '\0') // si on a un opérateur et que cet opérateur est un moins
            {
                ...
                else if (!(ast[i_anc]->type == TYPE_NUMBER || ast[i_anc]->type == TYPE_EXPRESSION || ast[i_anc]->type == TYPE_STRING || ast[i_anc]->type == TYPE_LISTINDEX || ast[i_anc]->type == TYPE_FONCTION || ast[i_anc]->type == TYPE_LIST || ast[i_anc]->type == TYPE_BOOL || ast[i_anc]->type == TYPE_CONST || ast[i_anc]->type == TYPE_NONE || ast[i_anc]->type == TYPE_EXCEPTION || ast[i_anc]->type == TYPE_PARENTHESE2 || ast[i_anc]->type == TYPE_VARIABLE || (ast[i_anc]->type==TYPE_OPERATOR && gramm1.tab[strlist_index(&operateurs3, tokenAdd->tab[i_anc])] == VARLEFT)) )
                {
...
void finsTypesComposes(int *debTok2, int *debTok3, int* lastDebTok, int k, _Bool *nouvTokComp, _Bool* isPotentiallyPar, _Bool *isPotentiallyFonc, int *foncStep, strlist* tokenAdd, intlist typeTok, int *nbPar, _Bool *isPotentiallyInst, int *instStep, int *listIndexStep, _Bool *isPotentiallyList, _Bool *isPotentiallyListIndex, int *nbCro, _Bool *isPotentiallyBlock, int *nbAcc, int *foncParDeb, int *listeParDeb, int *indexParDeb, Ast** ast, int offset)
{
    ...
    else if ((*isPotentiallyInst) && (*instStep)==2 && tokenAdd->tab[k][0] == ')' && (*nbPar)==0) // fin instruction de bloc
    {
        ...
        parse(&tokens2, types2, ast + *debTok2 + 2, offset + *debTok2 + 2);
    ...
    while (true)
    {
        ...
        if (funcMethStep == 2)
        {
            if (ast[i]->type == TYPE_BLOCK)
            {
                ...
                statements(&types2, &tokens2, ast + i + 1, offset + i + 1);
        ...
        if (isPotentiallyConBlock && conBlockStep == 1 && (nextindex >= tokens->len || (ast[nextindex]->type != TYPE_ENDOFLINE && ast[nextindex]->type != TYPE_BLOCKIF && ast[nextindex]->type != TYPE_BLOCKELSE && ast[nextindex]->type != TYPE_BLOCKELIF && ast[nextindex]->type != TYPE_BLOCK))) {
            // fin conblock si le type d'après ne correspond pas ou qu'on est à la fin
            ast_push_check(&ast[debConBlock], ast[i]->fin, TYPE_CONDITIONBLOCK);
        ...
        if (isPotentiallyTE && TEStep == 1 && (nextindex >= tokens->len || (ast[nextindex]->type != TYPE_BLOCKEXCEPT && ast[nextindex]->type != TYPE_ENDOFLINE))) {
            // on réunit les deux
            ast_push_check(&ast[debTE], ast[i]->fin, TYPE_TRYEXCEPT);
