...
void tree_destroy(Tree* tree)
{
    ...
    //printf("type arbre supprime : %d    %s\n", tree->type, tree->label1);
...
Tree* tree_copy(Tree* tree)
{
    ...
    Tree* tree2 = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
    if (CODE_ERROR != 0)
        return NULL;
...
void createExpressionTreeAux(Tree* tree, Ast** ast, strlist* tokens, int offset)
{
    ...
    if (count == real_length - 1) // traitement des cas de base de la fonction récursive
    {
        ...
        if (real_length > 1) { // on enlève tous les retours à la ligne
            ...
            createExpressionTreeAux(tree, ast + i, &tokens2, offset + i);
            return;
        }
        ...
        else if (ast[0]->type == TYPE_FONCTION)
        {
            ...
            if (argsAst != NULL && strlist_ast_inList(&argsTok, argsAst, ":", TYPE_OPERATOR, offset + lenNomFonc + 1))
            {
                ...
                while (i < argsTok.len - 2)
                {
                    ...
                    Tree* t = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                    if (CODE_ERROR != 0)
                    {
                        tree_destroy(t1);
                        return ;
                    ...
                    createExpressionTreeAux(t, argsAst + i, &tok2, offset + argsAst_offset + i);
            ...
            else
            {
                ...
                Tree* fonction = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);

                createExpressionTreeAux(fonction, ast, &tokens2, offset);
                ...
                Tree* argsTree = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                ...
                if (argsAst != NULL)
                {
                    ...
                    if (virgules)
                    {
                        createExpressionTreeAux(argsTree, argsAst, &argsTok, offset + lenNomFonc + 1);
                        if (CODE_ERROR != 0)
                        {
                            return;
                        }
                    ...
                    {
                        Tree* fils = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                        if (CODE_ERROR != 0)
                        {
                            return;
                        }
                        createExpressionTreeAux(fils, argsAst, &argsTok, offset + lenNomFonc + 1);
                        if (CODE_ERROR != 0)
                        {
                            tree_destroy(fils);
                            return;
        ...
        else if (ast[0]->type == TYPE_LISTINDEX) // un arbre listindex a deux fils : le fils gauche contient l'arbre correspondant à la liste et le fils droit l'arbre de l'index
        {
            ...
            Tree* expr = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
            createExpressionTreeAux(expr, indAst, &indTok, offset + lenNomInd + 1);
            ...
            Tree* nomListe = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);

            strlist nomTok;
            nomTok.tab = tokens->tab;
            nomTok.len = lenNomInd;

            createExpressionTreeAux(nomListe, ast, &nomTok, offset);
        ...
        else if (ast[0]->type == TYPE_LIST)
        {
            ...
            {
                Tree* fils = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                createExpressionTreeAux(fils, listeAst, &listeTok, offset + 1);
            ...
            // on doit donc effectuer un appel récursif dessus en enlevant l'étage EXPRESSION et en ignorant les potentielles parenthèses
            ast_pop(ast[0]);
            createExpressionTreeAux(tree, ast, tokens, offset);
            return;
        }
    ...
    else if (tokens->len > 1)
    {
        ...
        if (nbVirgules > 0)
        {
            ...
            for (int j=0 ; j <= nbVirgules ; j++)// boucle qui parcourt toutes les virgules
            {
                ...
                arbre = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);

                sous_tokens.tab = tokens->tab + debut;
                sous_tokens.len = index - debut;
                createExpressionTreeAux(arbre, ast + debut, &sous_tokens, offset + debut);
        ...
        else
        {
            ...
            if (typeOperande == RIGHT_LEFT)
            {
                ...
                if ((long int)number_toDouble(tree->label2) == 30) // si c'est un point
                {
                    ...
                    createExpressionTreeAux(tree, astDroite, &tokensDroite, offset + index + 1);
                    ...
                    createExpressionTreeAux(fils, astGauche, &tokensGauche, offset);
                ...
                else if ((long int)number_toDouble(tree->label2) == 34) // opérateur >>
                {
                    ...
                    if (ast[index+1]->type != TYPE_VARIABLE)
                    {
                        ...
                        createExpressionTreeAux(t, ast, &tokensGauche, offset);
                ...
                else
                {
                    ...
                    Tree* filsg = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                    if (CODE_ERROR != 0)
                    {
                        err_free(tree->label1);
                        tree->label1 = NULL;
                    ...
                    Tree* filsd = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                    if (CODE_ERROR != 0)
                    {
                        err_free(tree->label1);
                        tree->label1 = NULL;
                    ...
                    createExpressionTreeAux(filsg, ast, &tokensGauche, offset);
                    ...
                    createExpressionTreeAux(filsd, ast + index + 1, &tokensDroite, offset + index + 1);
                    if (CODE_ERROR != 0)
                    {
                        tree_destroy(filsg);
                        tree_destroy(filsd);
            ...
            else if (typeOperande == VARLEFT) // variable à gauche comme -- ou ++
            {
                ...
                createExpressionTreeAux(fils, ast, &ntokens, offset);
                if (CODE_ERROR != 0)
                {
                    tree_destroy(fils);
                    number_destroy(tree->label2);
            ...
            else if (typeOperande == VARRIGHT || typeOperande == RIGHT)
            {
                ...
                createExpressionTreeAux(fils, ast + 1, &ntokens, offset + 1);
                if (CODE_ERROR != 0)
                {
                    tree_destroy(fils);
                    return;
                ...
                if ((long int)number_toDouble(tree->label2) == 39 && fils->type != TYPE_FONCTION) { // pour parallel
                    tree_destroy(fils);
                    CODE_ERROR = 100;
                    return;
                }
...
void createExpressionTree(Tree* tree, char* string)
{
    ...
    createExpressionTreeAux(tree, ast, tokens, 0);
...
*/
void createStatementIEWFTree(Tree* tree, Ast** ast, strlist* tokens, int offset, int type)
{
    ...
    Tree* fils1 = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
    if (CODE_ERROR != 0)
    {
        return;
    }
    createExpressionTreeAux(fils1, exprAst, &exprToks, offset + 2);

    
    Tree* fils2 = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
    if (CODE_ERROR != 0)
    {
        tree_destroy(fils1);
        return;
    ...
    createSyntaxTreeAux(fils2, blocAst, &blocToks, offset + ast[0]->fin - offset + 2);
...
void createStatementElseTree(Tree* tree, Ast** ast, strlist* tokens, int offset)
{
    ast_pop(ast[0]);

    strlist bloc = (strlist) {.tab = tokens->tab + 2, .len = tokens->len - 3};
    
    createSyntaxTreeAux(tree, ast + 2, &bloc, offset + 2);
...
void createConditionBlockTree(Tree* tree, Ast** ast, strlist* tokens, int offset)
{
    ast_pop(ast[0]);
    ...
    while (i < tokens->len)
    {
        ...
        if (ast[i]->type == TYPE_STATEMENTELSE)
        {
            ...
            createStatementElseTree(fils, ast + i, &elseToks, offset + i);
        ...
        else
        {
            ...
            createStatementIEWFTree(fils, ast + i, &blocToks, offset + i, ast[i]->type);
            if (CODE_ERROR != 0)
            {
                tree_destroy(fils);
                return;
...
void createFunctionTree(Tree* tree, Ast** ast, strlist* tokens, int offset, _Bool isMethod)
{
    ...
    Tree* syntaxTree = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
    if (CODE_ERROR != 0) {
        return;
    }

    createSyntaxTreeAux(syntaxTree, ast + i + 1, &codeTok, offset + i + 1);
    ...
    Tree* opt_args = tree_create(NULL, number_default(), TYPE_SYNTAXTREE); // même si la fonction n'a pas d'arguments elle a un arbre
            ...
            if (strcmp(argsTok.tab[i], ",") != 0) // si ce n'est pas une virgule, c'est une variable (enfin on vérifie)
            {
                ...
                if (argsAst[i]->type == TYPE_VARIABLE)
                {
                    ...
                    if (ast_index(argsAst, i, args_offset) < ast_length(argsAst, argsTok.len, args_offset) - 2 && strcmp(argsTok.tab[argsAst[i]->fin - args_offset + 1], ":=") == 0) // argument optionnel
                    {
                        ...
                        Tree* exp = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                        createExpressionTreeAux(exp, argsAst + i + 2, &tokensarg, args_offset + i + 2);
                        ...
                        tree_appendSon(opt_args, tree_create(NULL, number_default(), TYPE_SYNTAXTREE));
                    }
                }
...
void createSyntaxTreeAux(Tree* tree, Ast** ast, strlist* tokens, int offset) {
    ...
    while (index < tokens->len) {
        ...
        if (ast[index]->type == TYPE_TRYEXCEPT) {
            ...
            Tree* try_except = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
            Tree* try_tree = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
            

            // try
            strlist tryTok = (strlist) {.tab = tokens->tab + index + 2, .len = ast[index]->fin - offset - index - 2};

            createSyntaxTreeAux(try_tree, ast + index + 2, &tryTok, offset + index + 2);
            ...
            while (i < index2) {
                ...
                Tree* except_tree = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                Tree* except_expr = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                Tree* except_bloc = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
                ...
                createSyntaxTreeAux(except_bloc, ast + ast[i]->fin - offset + 2, &blocToks, offset + ast[i]->fin - offset + 2);
                ...
                {
                    createExpressionTreeAux(except_expr, ast + i + 2, &exprToks, offset + i + 2);
            ...
            Tree* corps = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
            ...
            createSyntaxTreeAux(corps, ast + index + 2, &blocTok, offset + index + 2);
            ...
            if (argsTok.len > 0) // s'il y a au moins un argument
            {
                ...
                if (virgules)
                {
                    createExpressionTreeAux(fils, argsAst, &argsTok, offset + index + 2);
                    if (CODE_ERROR != 0)
                    {
                        tree_destroy(fils);
                        return;
                ...
                else
                {
                    ...
                    createExpressionTreeAux(fils2, argsAst, &argsTok, offset + index + 2);
                    if (CODE_ERROR != 0)
                    {
                        tree_destroy(fils);
                        tree_destroy(fils2);
            ...
            createFunctionTree(fils, ast + index, &stru, offset + index, ast[index]->type == TYPE_METHODDEF);
            ...
            createStatementIEWFTree(fils, ast + index, &stru, offset + index, ast[index]->type);
            if (CODE_ERROR != 0) {
                tree_destroy(fils);
                return;
            }
            ...
            strlist conBlock = (strlist) {.tab = tokens->tab + index, .len = ast[index]->fin - offset - index};
            Tree* st = tree_create(NULL, number_default(), TYPE_SYNTAXTREE);
            createConditionBlockTree(st, ast + index, &conBlock, offset + index);
            ...
            createExpressionTreeAux(expr, ast + exprStart, &exprToks, offset + exprStart);
            tree_appendSon(tree, expr);
...
void createSyntaxTree(Tree* tree, char* program)
{
    ...
    createSyntaxTreeAux(tree, ast, tokens, 0);
