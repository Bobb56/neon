...
NeObject* _affect(NeObject* op2, NeObject* op1)
{
    ...
    else
    {
        ...
        op1->data = op2->data;
        op1->refc = op2->refc;
        incr_count(op1); // augmente le compteur de références, qui est commun à op2 et à op1
        return neo_copy(op1); // on augmente encore le compteur le temps que va durer la copie
    }
    return NULL;
}
...
NeObject* _affectNone(NeObject* op1, NeObject* op2)
{
    ...
    {
        // le booleen veut dire que dans op1 il y a une référence directe au même NeObject* que op1
        _Bool bo = neobject_destroy(op1,false); // il a supprimé la liste de base, du coup on modifie l[0] mais c'est la dernière fois qu'on y a accès, donc fuite mémoire
        if (bo /*&& op1->refc - autoref == 1*/) // non je pense pas car l'enveloppe n'a rien à voir avec les références. Un NeObject peut avoir plein de références et son enveloppe doit être quand même libérée
        {
            err_free(op1);
        }
        else
        {
            ...
            op1->data = op2->data;
            op1->refc = op2->refc;
            incr_count(op1); // incrémentation du compteur de références
        }
...
void _affect2(NeObject* op1, NeObject* op2)
{
    ...
    else
    {
        ...
        op1->data = op2->data;
        op1->refc = op2->refc;
        incr_count(op1); // incrémentation du pointeur de références
        return;
    }
    return;
}
