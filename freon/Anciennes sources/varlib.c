...
NeObject* neo_copy(NeObject* neo)
{
    ...
    else if (neo->type == TYPE_LIST)
    {
        ...
        int len = neo_list_len(neo); //longueur de la liste à copier
        
        NeObject* liste = neo_list_create(len); //nouvelle liste
        for (int i=0 ; i < len ; i++)
        {
            ((NeList*)liste->data)->tab[i] = neo_copy(neo_list_nth(neo,i)); // on ajoute à la liste liste une copie de l'élément i de la liste neo
        }
        return liste;
    }
...
char* type(int type)
{
    ...
    if (type == TYPE_FONCTION)
        return "Fonction de base";

    if (type == TYPE_LIST)
        return "Liste";

    if (type == TYPE_USERFUNC)
        return "Fonction";

    if (type == TYPE_USERMETHOD)
        return "Methode";
